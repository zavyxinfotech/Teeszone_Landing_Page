      ...
      <div className="w-full px-6 sm:px-10 lg:px-16 xl:px-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {INDUSTRIES.map((ind) => (
            <div
              key={ind.id}
              className="stripe-card stripe-card-hover rounded-3xl bg-white/90 backdrop-blur-xl border border-slate-200/80 p-6 shadow-md flex flex-col justify-between group"
            >
              <div className="space-y-4">
                {/* Visual Header Image */}
                <div className="relative aspect-[16/9] rounded-2xl overflow-hidden bg-slate-100 border border-slate-200/80 shadow-xs">
                  <img
                    src={ind.image}
                    alt={ind.name}
                  ...
                  />
                  <div className="absolute bottom-2 right-2 bg-[#0A2540]/90 backdrop-blur-md text-[10px] font-bold text-white px-2.5 py-1 rounded-lg border border-slate-700 shadow-md">
                    {ind.clientCount}
                  </div>
                </div>
                ...
                <div className="pt-3 border-t border-slate-200/80 space-y-2 font-inter">
